    ...
    public:
        ...
        void BeginSession(const std::string& name, const std::string& filepath = "results.json")
        {
            ...
            if (m_CurrentSession)
            {
                ...
                {
                    CRYSTAL_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
                }
                InternalEndSession();
            }
            ...
            else
            {
                if (Log::GetCoreLogger())
                {
                    CRYSTAL_CORE_ERROR("Instrumentor could not open result file '{0}'.", filepath);
                }
            }
        }
...
#define CRYSTAL_PROFILE 0
        ...
        #define CRYSTAL_FUNC_SIG __func__
    #else
        #define CRYSTAL_FUNC_SIG "CRYSTAL_FUNC_SIG unknown!"
    #endif
